...
RUN set -x && \
    ...
    echo "deb http://repo.zabbix.com/zabbix/${ZABBIX_VERSION}/debian jessie main" >>/etc/apt/sources.list && \
    echo "deb-src http://repo.zabbix.com/zabbix/${ZABBIX_VERSION}/debian jessie main" >>/etc/apt/sources.list && \
    apt-get update && \
    apt-get install -y --no-install-recommends \
            zabbix-agent && \
    ...
    apt-get clean -y && \
    rm -rf /var/lib/apt/lists/* /root/.gnupg /var/log/* /etc/logroate.d/* && \
    mkdir -p /assets/cron && \
    rm -rf /etc/timezone && \
